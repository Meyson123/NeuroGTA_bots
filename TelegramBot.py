...
import aiohttp
import os
from myConfig import  mongodb_address, AdminTgId, NeedTopicDelay, TopicDelayTg, TopicPriority, AdminTgSecondId, default_topic_suggest_message
import time
...
async def tema(message):
    if message.text[6:] =='':
        ...
        sourse = 'Telegram'
        if message.chat.id != AdminTgId and message.chat.id != AdminTgSecondId:
                if NeedTopicDelay:
                    ...
                    if message.chat.id in last_topic_time and time.time() - last_topic_time[message.chat.id] < TopicDelayTg:
                        minuta = "минуту" if TopicDelayTg / 60 == 1 else ("минуты" if 2 <= TopicDelayTg / 60 <= 4 else "минут")
                        await bot.reply_to(message,f"Вы можете добавить тему не чаще, чем раз в {int(TopicDelayTg / 60)} {minuta}." )
                        return
