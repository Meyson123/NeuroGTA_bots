...
from myConfig import mongodb_address, TopicDelay,MashupDelay, CanAddTopic, CanAddMashup, NeedTopicDelay, NeedMashupDelay, NeedMashupDelayPerUser, TopicsChatName, MashupsChatName, AdminNames, valid_speakers, TopicPriority, MashapPriority, replacements, default_topic_suggest_message, default_style
        ...
        try:
            if not (message.author.name in AdminNames):
                if NeedTopicDelay:
                    ...
                    if message.author.name in last_topic_time and time.time() - last_topic_time[message.author.name] < TopicDelay:
                        minuta = "минуту" if TopicDelay / 60 == 1 else ("минуты" if 2 <= TopicDelay / 60 <= 4 else "минут")
                        await message.reply(f"Вы можете добавить тему не чаще, чем раз в {int(TopicDelay / 60)} {minuta}.")
                        return
        ...
        try:
                ...
                if NeedMashupDelay:
                    ...
                    if NeedMashupDelayPerUser:
                        ...
                        if message.author.name in last_mashup_time_per_user and time.time() - last_mashup_time_per_user[message.author.name] < MashupDelay:
                            minuta = "минуту" if MashupDelay / 60 == 1 else ("минуты" if 2 <= MashupDelay / 60 <= 4 else "минут")
                            await message.reply(f"Вы можете добавить мэшап не чаще, чем раз в {int(MashupDelay / 60)} {minuta}.")
                            return
                    #Если задержка общая
                    else:
                        if time.time() - last_mashup_time_single < MashupDelay:
                            minuta = "минуту" if MashupDelay / 60 == 1 else ("минуты" if 2 <= MashupDelay / 60 <= 4 else "минут")
                            await message.reply(f"В экспериментальном режиме работает общая задержка на всех юзеров. Мэшап может быть добавлен не чаще чем раз в {int(MashupDelay / 60)} {minuta}.")
                            ...
                            await message.reply(f"До возможности добавить следующий мэшап: {min_ost} {minuta_ost_text}.")
                            return
            ...
            if mashup and " " in mashup:
                ...
                if not(url.startswith('https://www.youtube.com/watch?v=')):
                    await message.reply('Используйте ссылку на видео на Ютубе, начинающуюся с https://www.youtube.com/watch?v=', mention_author=False)
                    return;
